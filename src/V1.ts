...
const forbiddenSendHeaders = ["connection", "content-length", "transfer-encoding"];
...
const tunnelRequest: RouteCallback = async (request, res, options) => {
  ...
  const response = await bareFetch(request, abort.signal, headers, remote, options);
...
const tunnelSocket: SocketRouteCallback = async (request, socket, head, options) => {
  const abort = new AbortController();
